...
* **FR-1.4**: Tenant isolation enforcing individual data namespaces (`public.current_athlete_email()`).
...
* **FR-8.2**: Interactive route mapping using open-source Leaflet / OpenStreetMap fallback.
...
* **FR-9.2**: Context-grounded response generation using Google Gemini 1.5 Flash.
...
* **FR-10.1**: Cloud database synchronization with Supabase PostgreSQL backend.
* **FR-10.2**: Offline-first persistent client cache with automatic queue flush upon network reconnection.
...
| **Security** | Database Tenant Isolation | 100% enforced via PostgreSQL Row-Level Security |
