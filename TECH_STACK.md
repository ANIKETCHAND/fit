...
| **ORM / Schema** | Drizzle ORM | `^0.44.7` | High-speed TypeScript ORM for schema migration and type generation. |
| **Generative AI** | Google Gemini 1.5 Flash | REST v1beta | Conversational coaching intelligence, diet creation, workout generation. |
...
* **Database-Enforced Row-Level Security (RLS)**: Security is enforced inside the PostgreSQL engine (`public.current_athlete_email()`), eliminating accidental security holes in application code.
...
# Optional Custom Gemini API Key (Defaults to embedded runtime fallback)
