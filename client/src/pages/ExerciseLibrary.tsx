...
export default function ExerciseLibrary() {
  ...
  return <WorkflowLayout kicker="Training / movement library" title="Study the pattern" detail="Set your prescription, review the coaching cues, then mark each completed movement directly in the active workout."><section className="library-toolbar"><div className="library-search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search movement, focus, or equipment" /></div><div className="library-filters"><Filter size={14} />{["All signals", "Favorites", "Coached", "Pectorals", "Quadriceps", "Biceps", "Lats"].map((item) => <button key={item} className={focus === item ? "selected" : ""} onClick={() => setFocus(item)}>{item}</button>)}</div><button className="library-tune" onClick={() => toast("Movement filters calibrated for equipment and training block")}><SlidersHorizontal size={16} />Tune library</button></section><section className="library-scan-anchor" aria-label="Movement scan reference"><div className="anchor-body"><i className="anchor-head" /><i className="anchor-torso" /><i className="anchor-pec left" /><i className="anchor-pec right" /><i className="anchor-core" /></div><div><span className="panel-label">Live movement scan</span><b>Selected pattern routes through <em>pectoral / triceps</em> signal path.</b></div><div className="anchor-readouts"><span><i />Joint line stable</span><span><i />Range 86%</span><span><i />Load ready</span></div></section><section className="exercise-library-grid">{filtered.map((exercise, index) => <ExerciseFlipCard key={exercise.id} exercise={exercise} index={index} onStage={stage} favorite={Boolean(preferenceFor(exercise.id).favorite)} viewed={Boolean(preferenceFor(exercise.id).viewedAt)} target={targetFor(exercise)} onTargetChange={(target) => updateTarget(exercise, target)} onCompletionChange={(completed) => setCompleted(exercise, completed)} onFavorite={() => toggleFavorite(exercise.id)} onHide={() => hideExercise(exercise.id)} onViewed={() => markViewed(exercise.id)} />)}</section>{filtered.length === 0 && <div className="library-empty">No movement signal matches this filter. Reset the scan or broaden the search.</div>}</WorkflowLayout>;
}
