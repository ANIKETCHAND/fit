...
export default function Profile() {
  ...
  const [tab, setTab] = useState<"overview" | "activity" | "compare">("overview");
  ...
  return <div className="app-shell profile-shell">
    ...
    <main className="profile-main">
      ...
      <motion.section className="profile-identity-bar" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .32 }}>
        <div className="identity-athlete"><div className={`profile-orb ${athlete.photoDataUrl ? "has-photo" : ""}`}>{athlete.photoDataUrl ? <img src={athlete.photoDataUrl} alt={`${athlete.name} profile`} /> : initials(athlete.name)}</div><div><span className="panel-label">Athlete record</span><strong>{athlete.name}</strong><p>{athlete.location || "Local profile"} · <b>{athlete.focus}</b></p></div><button className="profile-edit-action" onClick={openProfileEditor}><Pencil size={13} /> Edit profile</button></div><div className="profile-mark-stamp" aria-hidden="true"><img src="/manus-storage/fittrack-signal-mark_e3117665.png" alt="" /><span>Signal / identity</span></div>
        <div className="profile-tabs" role="tablist" aria-label="Profile analytics views">{(["overview", "activity", "compare"] as const).map((entry) => <button key={entry} role="tab" aria-selected={tab === entry} className={tab === entry ? "active" : ""} onClick={() => { setTab(entry); if (entry !== "overview") toast(`${entry === "activity" ? "Activity ledger" : "Performance comparison"} view is being calibrated.`); }}>{entry === "overview" ? "Signal overview" : entry === "activity" ? "Activity ledger" : "Compare blocks"}</button>)}</div>
